...
def metrics_from_scores_stream(score_fn, obs, candidates, eval_batch: int, ks=(5, 10, 20)):
    ...
    for i in tqdm(range(0, obs.shape[0], eval_batch), desc="Eval", leave=False):
        ...
        # number of candidates strictly better than positive
        ...
        for k in ks:
            ...
            sums[f"NDCG@{k}"] += (
                (hit / torch.log2(rank.float() + 1.0)).sum().item()
            )
...
def validate_candidates(candidates: torch.Tensor, num_items: int, tag: str):
    ...
    if cmax >= num_items:
        raise ValueError(
            f"{tag}: candidate id {cmax} >= num_items ({num_items}). "
            "Candidates were generated with a different vocabulary."
        )
...
def main():
    ...
    obs, candidates, dataset_num_items = load_split(
        dataset_dir / f"rl_{split}.npz",
        dataset_dir / f"{split}_cands_rl.npy",
    )

    validate_candidates(candidates, dataset_num_items, "DATASET")
    ...
    if bc_ckpt:
        ...
        validate_candidates(candidates, num_items, "BC")
        ...
        results["BC"] = metrics_from_scores_stream(
            bc_score_fn, obs, candidates, eval_batch
        )
    ...
    if iql_ckpt:
        ...
        validate_candidates(candidates, num_items, "IQL")
        ...
        def q_score_fn(ob, cb):
            ...
            for j in range(0, C, cand_chunk):
                ...
                out[:, j:j + c] = q_flat.cpu()

            return out


        results["IQL"] = metrics_from_scores_stream(
            q_score_fn, obs, candidates, eval_batch
        )
    ...
    if awr_ckpt:
        ...
        validate_candidates(candidates, num_items, "AWR")
        ...
        results["AWR"] = metrics_from_scores_stream(
            awr_score_fn, obs, candidates, eval_batch
        )
